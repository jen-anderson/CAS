...
async function getOne(id: string, ctx: any) {
  return await ctx.supabase.from("references").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: any) {
  let dbQuery = ctx.supabase.from("references").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: any) {
  return await ctx.supabase.from("references").insert(body);
}

async function update(id: string, body: any, ctx: any) {
  return await ctx.supabase.from("references").update(body).eq("id", id);
}

async function remove(id: string, ctx: any) {
  return await ctx.supabase.from("references").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function referenceHandler(req: Request, method: string, ctx: any) {
