...
async function getOne(id: string, ctx: AppContext) {
  return await ctx.supabase.from("alias_reference").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: AppContext) {
  let dbQuery = ctx.supabase.from("alias_reference").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: AppContext) {
  return await ctx.supabase.from("alias_reference").insert(body);
}

async function update(id: string, body: any, ctx: AppContext) {
  return await ctx.supabase.from("alias_reference").update(body).eq("id", id);
}

async function remove(id: string, ctx: AppContext) {
  return await ctx.supabase.from("alias_reference").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function alias_referenceHandler(req: Request, method: string, ctx: AppContext) {
