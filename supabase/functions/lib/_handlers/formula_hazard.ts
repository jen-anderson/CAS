...
async function getOne(id: string, ctx: AppContext) {
  return await ctx.supabase.from("formula_hazard").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: AppContext) {
  let dbQuery = ctx.supabase.from("formula_hazard").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: AppContext) {
  return await ctx.supabase.from("formula_hazard").insert(body);
}

async function update(id: string, body: any, ctx: AppContext) {
  return await ctx.supabase.from("formula_hazard").update(body).eq("id", id);
}

async function remove(id: string, ctx: AppContext) {
  return await ctx.supabase.from("formula_hazard").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function formula_hazardHandler(req: Request, method: string, ctx: AppContext) {
