...
async function getOne(id: string, ctx: AppContext) {
  return await ctx.supabase.from("hazard_pcode").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: AppContext) {
  let dbQuery = ctx.supabase.from("hazard_pcode").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: AppContext) {
  return await ctx.supabase.from("hazard_pcode").insert(body);
}

async function update(id: string, body: any, ctx: AppContext) {
  return await ctx.supabase.from("hazard_pcode").update(body).eq("id", id);
}

async function remove(id: string, ctx: AppContext) {
  return await ctx.supabase.from("hazard_pcode").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function hazard_pcodeHandler(req: Request, method: string, ctx: AppContext) {
