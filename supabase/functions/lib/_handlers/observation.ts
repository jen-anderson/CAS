...
async function getOne(id: string, ctx: AppContext) {
  return await ctx.supabase.from("observation").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: AppContext) {
  let dbQuery = ctx.supabase.from("observation").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: AppContext) {
  return await ctx.supabase.from("observation").insert(body);
}

async function update(id: string, body: any, ctx: AppContext) {
  return await ctx.supabase.from("observation").update(body).eq("id", id);
}

async function remove(id: string, ctx: AppContext) {
  return await ctx.supabase.from("observation").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function observationHandler(req: Request, method: string, ctx: AppContext) {
