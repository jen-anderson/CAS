...
async function getOne(id: string, ctx: AppContext) {
  return await ctx.supabase.from("pcode_group_map").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: AppContext) {
  let dbQuery = ctx.supabase.from("pcode_group_map").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: AppContext) {
  return await ctx.supabase.from("pcode_group_map").insert(body);
}

async function update(id: string, body: any, ctx: AppContext) {
  return await ctx.supabase.from("pcode_group_map").update(body).eq("id", id);
}

async function remove(id: string, ctx: AppContext) {
  return await ctx.supabase.from("pcode_group_map").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function pcode_group_mapHandler(req: Request, method: string, ctx: AppContext) {
