...
async function getOne(id: string, ctx: AppContext) {
  return await ctx.supabase.from("property_type").select("*").eq("id", id).single();
}

async function search(query: string | null, ctx: AppContext) {
  let dbQuery = ctx.supabase.from("property_type").select("*");
  if (query) dbQuery = dbQuery.ilike("name", `%${query}%`);
  return await dbQuery;
}

async function add(body: any, ctx: AppContext) {
  return await ctx.supabase.from("property_type").insert(body);
}

async function update(id: string, body: any, ctx: AppContext) {
  return await ctx.supabase.from("property_type").update(body).eq("id", id);
}

async function remove(id: string, ctx: AppContext) {
  return await ctx.supabase.from("property_type").delete().eq("id", id);
}

// Main Handler (Traffic Controller) ---
export async function property_typeHandler(req: Request, method: string, ctx: AppContext) {
